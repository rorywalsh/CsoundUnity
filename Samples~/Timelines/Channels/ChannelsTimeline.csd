<Cabbage>
form caption("Channels Sample Timeline") size(570, 220), guiMode("queue"), pluginId("chtl")
...
rslider bounds(280, 10, 80, 80) channel("release")   range(0.001, 1.0, 0.1,   1, 0.001) text("Release")
rslider bounds(370, 10, 80, 80) channel("cutoff")    range(200, 12000, 4000,  1, 1)     text("Cutoff")
rslider bounds(460, 10, 80, 80) channel("reverbMix") range(0, 1, 0.2, 1, 0.001)         text("Reverb")
rslider bounds(10, 110, 80, 80) channel("resonance") range(0, 1, 0.3, 1, 0.001)         text("Resonance")
...
rslider bounds(280,110, 80, 80) channel("panSpread") range(0, 1, 0.3, 1, 0.01)          text("Pan Spread")
...
<CsInstruments>
; Channels Sample Timeline — versatile instrument for all Score modes
; p4 = frequency in Hz
...
instr 1
    ...
    ; All parameters are i-rate (read once at note start) except cutoff and resonance
    ; which are k-rate so that Timeline channel animation affects them in real-time.
    iGain    = chnget:i("gain")
    iAmp     = chnget:i("amp")
    ...
    ; k-rate: respond in real-time to Timeline channel automation
