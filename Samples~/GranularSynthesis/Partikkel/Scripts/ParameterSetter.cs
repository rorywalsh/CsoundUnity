...
namespace Csound.Unity.Samples.GranularSynthesis.Partikkel
{
    public class ParameterSetter : MonoBehaviour
    {
        ...
        public void Init(CsoundUnity csound, GameObject player, CsoundChannelController xChannel, CsoundChannelController zChannel, Vector2 minPos, Vector2 maxPos)
        {
            ...
            var xPos = RU.Remap(xChannel.value, xChannel.min, xChannel.max, minPos.x, maxPos.x);
            var zPos = zChannel != null ? RU.Remap(zChannel.value, zChannel.min, zChannel.max, minPos.y, maxPos.y) : 0;
        ...
        void Update()
        {
            if (transform.position != _lastPos)
            {
                ...
                if (_zChan != null)
                {
                    var valZ = RU.Remap(pos.z, _minPos.y, _maxPos.y, _zChan.min, _zChan.max, true);
        ...
        private void RemapCanvas(Vector3 _playerPos)
        {
            ...
            var remap = RU.Remap(dist, 0, Vector3.Distance(_minPos, _maxPos), _canvasScaleRange.x, _canvasScaleRange.y);
