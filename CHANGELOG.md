...
- [Add] Copy / Paste of control channels through the system clipboard: right-click the Control Channels foldout, or call CopyChannelsToClipboard() / PasteChannelsFromClipboard() at runtime. Uses the same JSON a preset is saved in, so a clipboard snapshot and a .json preset are interchangeable. Pasting from a different csd applies the channels whose names match and reports the rest, rather than refusing outright as SetPreset does
...
- [Fix] Selecting a CsoundUnityChild logged bursts of NullReferenceException from Unity's built-in audio filter meter. The meter is still drawn — the exception came from its final request to repaint itself, on a layout pass that paints nothing
...
- [Fix] After any script recompile, looking a channel up by name silently found nothing in the editor: the name→index map is not serialized, so a domain reload left it empty while the channel list survived, and neither SetCsd nor Init runs again just because scripts were rebuilt. SetChannel and GetChannelController did nothing at all in that state. The map is now rebuilt on demand
